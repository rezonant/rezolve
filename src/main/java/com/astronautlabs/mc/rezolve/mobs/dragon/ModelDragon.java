...
public class ModelDragon extends ModelBase
{
	...
	public void render(Entity entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scale)
	{
		...
		float currentTime = entitydragon.prevAnimTime + (entitydragon.animTime - entitydragon.prevAnimTime) * this.partialTicks;
		this.jaw.rotateAngleX = (float)(Math.sin((double)(currentTime * ((float)Math.PI * 2F))) + 1.0D) * 0.2F;
		float flapFactor = (float)(Math.sin((double)(currentTime * ((float)Math.PI * 2F) - 1.0F)) + 1.0D);
		flapFactor = (flapFactor * flapFactor + flapFactor * 2.0F) * 0.05F;
		GlStateManager.translate(0.0F, flapFactor - 2.0F, -3.0F);
		GlStateManager.rotate(flapFactor * 2.0F, 1.0F, 0.0F, 0.0F);

		double[] adouble = entitydragon.getMovementOffsets(6, this.partialTicks);
		float f6 = this.simplifyAngle(entitydragon.getMovementOffsets(5, this.partialTicks)[0] - entitydragon.getMovementOffsets(10, this.partialTicks)[0]);
		float f7 = this.simplifyAngle(entitydragon.getMovementOffsets(5, this.partialTicks)[0] + (double)(f6 / 2.0F));

		float f8 = currentTime * ((float)Math.PI * 2F);
		...
		for (int i = 0; i < 5; ++i) {
			...
			this.spine.rotateAngleY = this.simplifyAngle(adouble1[0] - adouble[0]) * 0.017453292F * 1.5F;
			this.spine.rotateAngleZ = -this.simplifyAngle(adouble1[0] - (double)f7) * 0.017453292F * 1.5F;
			this.spine.rotationPointX = tBoneX;
			this.spine.rotationPointY = tBoneY;
			this.spine.rotationPointZ = tBoneZ;
			this.spine.render(scale);

			tBoneX = (float)((double)tBoneX - Math.sin((double)this.spine.rotateAngleY) * Math.cos((double)this.spine.rotateAngleX) * 10.0D);
			tBoneY = (float)((double)tBoneY + Math.sin((double)this.spine.rotateAngleX) * 10.0D);
			tBoneZ = (float)((double)tBoneZ - Math.cos((double)this.spine.rotateAngleY) * Math.cos((double)this.spine.rotateAngleX) * 10.0D);
		}

		this.head.rotationPointX = tBoneX;
		this.head.rotationPointY = tBoneY;
		this.head.rotationPointZ = tBoneZ;
		double[] adouble2 = entitydragon.getMovementOffsets(0, this.partialTicks);

		//this.head.rotateAngleX = this.simplifyAngle((double)entitydragon.getHeadPartYOffset(6, adouble, adouble2)) * 0.017453292F * 1.5F * 5.0F;
		//this.head.rotateAngleY = this.simplifyAngle(adouble2[0] - adouble[0]) * 0.017453292F;
		//this.head.rotateAngleZ = -this.simplifyAngle(adouble2[0] - (double)f7) * 0.017453292F;

		this.head.rotateAngleX = entitydragon.rotationHeadPitch * 0.017453292F;
		this.head.rotateAngleY = -(float)(2 * Math.PI - entitydragon.rotationHeadYaw2 * 0.017453292F) - (float)Math.PI/2.0f;
		this.head.rotateAngleZ = 0f;

		//entitydragon.log("   °: {}, {}", entitydragon.rotationHeadPitch, entitydragon.rotationHeadYaw2);
		entitydragon.log("   π: {}, {}", this.head.rotateAngleX, this.head.rotateAngleY);
		...
		for (int j = 0; j < 2; ++j) {
			...
			float pi2 = currentTime * ((float)Math.PI * 2F);
			this.wing.rotateAngleX = 0.125F - (float)Math.cos((double)pi2) * 0.2F;
			this.wing.rotateAngleY = 0.25F;
			this.wing.rotateAngleZ = (float)(Math.sin((double)pi2) + 0.125D) * 0.8F;
			this.wingTip.rotateAngleZ = -((float)(Math.sin((double)(pi2 + 2.0F)) + 0.5D)) * 0.75F;
			this.rearLeg.rotateAngleX = 1.0F + flapFactor * 0.1F;
			this.rearLegTip.rotateAngleX = 0.5F + flapFactor * 0.1F;
			this.rearFoot.rotateAngleX = 0.75F + flapFactor * 0.1F;
			this.frontLeg.rotateAngleX = 1.3F + flapFactor * 0.1F;
			this.frontLegTip.rotateAngleX = -0.5F - flapFactor * 0.1F;
			this.frontFoot.rotateAngleX = 0.75F + flapFactor * 0.1F;
		...
		float f10 = -((float)Math.sin((double)(currentTime * ((float)Math.PI * 2F)))) * 0.0F;
		f8 = currentTime * ((float)Math.PI * 2F);
		tBoneY = 10.0F;
		tBoneZ = 60.0F;
		tBoneX = 0.0F;
		...
		for (int k = 0; k < 12; ++k) {
			...
			this.spine.rotateAngleY = (this.simplifyAngle(adouble2[0] - adouble[0]) * 1.5F + 180.0F) * 0.017453292F;
			this.spine.rotateAngleX = f10 + (float)(adouble2[1] - adouble[1]) * 0.017453292F * 1.5F * 5.0F;
			this.spine.rotateAngleZ = this.simplifyAngle(adouble2[0] - (double)f7) * 0.017453292F * 1.5F;
			this.spine.rotationPointY = tBoneY;
			this.spine.rotationPointZ = tBoneZ;
			this.spine.rotationPointX = tBoneX;
			tBoneY = (float)((double)tBoneY + Math.sin((double)this.spine.rotateAngleX) * 10.0D);
	 ...
	 * 180 to 180
	 */
	private float simplifyAngle(double value)
	{
		// NEW WAY

		return (float)((180.0D + value) % 360.0D - 180.0D);
