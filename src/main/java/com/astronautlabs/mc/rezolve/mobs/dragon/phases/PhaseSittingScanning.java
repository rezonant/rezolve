...
public class PhaseSittingScanning extends PhaseSittingBase
{
	...
	public void doLocalUpdate()
	{
		...
		if (entitylivingbase != null)
		{
			...
			{
				this.dragon.getPhaseManager().setPhase(DragonPhaseList.SITTING_ATTACKING);
			}
			else
			{
		...
		else if (this.scanningTime >= 100)
		{
			...
			if (entitylivingbase != null)
			{
				this.dragon.log("Charging player!");
				this.dragon.getPhaseManager().setPhase(DragonPhaseList.CHARGING_PLAYER);
				((PhaseChargingPlayer)this.dragon.getPhaseManager().getPhase(DragonPhaseList.CHARGING_PLAYER)).setTarget(new Vec3d(entitylivingbase.posX, entitylivingbase.posY, entitylivingbase.posZ));
			}
		}
	}
	...
	public DragonPhaseList<PhaseSittingScanning> getPhaseList()
	{
		return DragonPhaseList.SITTING_SCANNING;
	}
}