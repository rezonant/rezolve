...
public abstract class TorchWidget extends GuiComponent implements Widget, GuiEventListener, NarratableEntry, EventEmitter {
    ...
    /**
     * Translate the given point from screen space to local space
     * @param point
     * @return
     */
    public Point screenToLocal(Point point) {
        var rect = getScreenRect();
        return new Point(point.x - rect.getX(), point.y - rect.getY());
    }

    /**
     * Translate the given point from screen space to units relative to the bounds of the parent of this widget
     * @param point
     * @return
     */
