...
public class IconButton extends WidgetBase {
    ...
    protected void renderContents(PoseStack pPoseStack, int pMouseX, int pMouseY, float pPartialTick) {
        if (isHovered()) {
            RezolveGuiUtil.colorQuad(pPoseStack, 0xFFFFFFFF, x, y, width, height);
        }
        RezolveGuiUtil.colorQuad(pPoseStack, 0xFF000000, x + 1, y + 1, width - 2, height - 2);
        RezolveGuiUtil.textureQuad(pPoseStack, icon, x + 1, y + 1, width - 2, height - 2, 0, 0, 1, 1);
    }
